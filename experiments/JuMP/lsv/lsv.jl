...
function get_maxls_model(
    ...
    @variable(model, p_lb[i]           .<= pd0_var[i=1:gm.nb] .<= p_ub[i])
    @variable(model, q_lb[i]           .<= qd0_var[i=1:gm.nb] .<= q_ub[i])
    @variable(model, bounds[:risk_lb]   <= risk[i=1:gm.nl]     <= bounds[:risk_ub])
    @variable(model, bounds[:alpha_lb]  <= alpha               <= bounds[:alpha_ub])
    ...
    for var in x
        midpoint = 0.5 * (JuMP.upper_bound(var) + JuMP.lower_bound(var))
        JuMP.set_start_value(var, midpoint)
    end

    if xstart !== nothing
        for var in JuMP.all_variables(model)
            name = JuMP.name(var)
            if name in keys(xstart)
                JuMP.set_start_value(var, xstart[name])
            else
                println("WARNING: No start value for $var")
            end
        end
    end
    ...
    scalex = x -> (x .- mean) ./ std
    xn_lb = scalex(JuMP.lower_bound.(x))
    xn_ub = scalex(JuMP.upper_bound.(x))
    xn_start = scalex(JuMP.start_value.(x))
    xn = @variable(model,
        xn_lb[i] <= xn[i=1:length(x)] <= xn_ub[i], start=xn_start[i]
        #JuMP.lower_bound(x[i]) <= xn[i = 1:length(x)] <= JuMP.upper_bound(x[i]),
        #start=(JuMP.start_value(x[i]) - mean[i]) / std[i]
    )
    @constraint(model, xn .== (x .- mean) ./ std)
...
function get_maxls_model(
    ...
    model, formulation = get_maxls_model(
        gm, bounds, nnfile;
        normalize, gray_box, xstart
    )
