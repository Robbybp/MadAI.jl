...
FILEDIR = @__DIR__
MNISTDIR = joinpath(dirname(@__DIR__), "nn", "MNIST", "raw")
...
function get_adversarial_model(
    ...
    test_data = MLDatasets.MNIST(; split = :test, dir = MNISTDIR)
...
#        pivot_indices = MadAI.get_kkt_indices(m, variables, constraints)
...
#        blocks = MadAI.partition_indices_by_layer(m, formulation)
