...
function get_scopf_model(nnfile::String; sample_index = 1, xstart = nothing, kwargs...)
    ...
    pm, info = build_scopf(pmdata; stability_surrogate, surrogate_params, xstart)
    return (;
        model = pm.model,
...
function build_scopf(
    ...
    if stability_surrogate !== nothing
        ...
        if xstart !== nothing
            for var in JuMP.all_variables(pm.model)
                ...
                if name in keys(xstart)
                    ...
                    println("WARNING: No start value for variable $var")
                end
            end
        end
        ...
        # If we don't explicitly set these names, the gray-box and full-space
        # outputs will have different names from MathOptAI.
        for i in eachindex(outputs)
            JuMP.set_name(outputs[i], "y[$i]")
        end
    ...
    #if xstart !== nothing
    #    for (name, value) in xstart
    #        if !(startswith(name, "MathOptAI") || startswith(name, "y"))
    #            continue
    #        end
    #        var = JuMP.variable_by_name(pm.model, name)
    #        if var === nothing
    #            println("WARNING: No variable exists with name $name")
    #        else
