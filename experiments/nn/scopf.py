...
def _get_fname(nodes, layers, activation):
    return f"scopf-{activation}{nodes}nodes{layers}layers.pt"


def create_nn_architecture(nodes, layers, activation, load_weights=True):
    """Create an MLP with `layers` including input and output layers."""
    Activation = ACTIVATION_LOOKUP[activation]
    dimensions = [INPUT_DIM]
    dimensions.extend([nodes] * (layers - 2))
    dimensions.append(OUTPUT_DIM)
    network_layers = []
    for i, dim in enumerate(dimensions[:-1]):
        if i != 0:
            network_layers.append(Activation())
        network_layers.append(torch.nn.Linear(dim, dimensions[i + 1]))
    nn = torch.nn.Sequential(*network_layers)
    if load_weights:
        weightfile = os.path.join(WEIGHTSDIR, _get_fname(nodes, layers, activation))
        nn.load_state_dict(torch.load(weightfile))
    return nn
...
def main(args):
    ...
    assert sim_inputs.shape[1] == INPUT_DIM
    assert freq_output.shape[1] == OUTPUT_DIM
    ...
    if args.small:
        ...
        nn = create_nn_architecture(
            args.nodes, args.layers, args.activation, load_weights=False
        )
        #    torch.nn.Linear(input_dim, 500),
        #    torch.nn.Tanh(),
