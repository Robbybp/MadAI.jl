...
#include("../src/btsolver.jl")
#include("../src/formulation.jl")
#include("../src/nlpmodels.jl")
#include("../src/kkt-partition.jl")
...
# At some point, MadNLP changed from solve! to solve_linear_system!...
# If we can't find one method for a specific LinearSolver, try the other...
# Leads to infinite recursion if we can't find either???
function MadNLP.solve_linear_system!(solver::MadNLP.AbstractLinearSolver, rhs::Vector)
...
function solve_kkt_old(
    ...
    # MadNLP must always use the same linear solver, otherwise the matrices
    # we end up with will be slightly different between different solvers.
    #display(opt_linear_solver)
    #fields = fieldnames(typeof(opt_linear_solver))
    #madnlp_opt = Dict(zip(fields, map(f -> getproperty(opt_linear_solver, f), fields)))
    #display(madnlp_opt)
    madnlp = MadNLP.MadNLPSolver(
        nlp;
        tol = 1e-6,
    ...
    matrix = MadNLP.get_kkt(madnlp.kkt)
    _t = time()
    ...
    results = Any[]
    # The values in `iterates` are intentionally unused. Its length specifies
    # how many MadNLP iterations to advance for this comparison.
    nsamples = length(iterates)
    for i in 1:nsamples
        # Solve with MadNLP up to max_iter
        MadNLP.regular!(madnlp)
        # *After the solve* advance the iteration count
